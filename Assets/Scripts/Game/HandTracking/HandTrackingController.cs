...
namespace Game.HandTracking
{
  ...
  ///
  /// NOTE: 一時的にMediaPipe GestureRecognizer(gesture_recognizer.bytes)へ切り替えたが、
  /// 同梱のネイティブライブラリとモデルのバージョン不整合で
  /// 「BadStatusException: NOT_FOUND: No file with name: hand_landmarker.task」が発生し
  /// ロードに失敗したため、確実に動作するHandLandmarkerへ戻した。ジェスチャー判定は
  ...
  public class HandTrackingController : MonoBehaviour
  {
    ...
    [Tooltip("手モデルの表示可否（ゲーム世界に入るまでは非表示）")]
    ...
    private void Awake()
    {
      ...
      if (_leftHandPrefab != null)
      {
        ...
        _leftHandInstance.name = "Hand_Left";
        ...
        _rightHandInstance.name = "Hand_Right";
    ...
    /// 手モデルのボーン・回転計算を一切経由しないため、UIカーソルの位置決めに使うと安定する。
    ...
    /// </summary>
    private Vector2 ToViewport(NormalizedLandmark lm) => NormalizedToViewport(lm.x, lm.y);

    /// <summary>ToViewportの公開版。MediaPipeの正規化座標(0〜1)を直接渡せるようにし、外部(戦闘中の
    /// カーソル・全体コマンド入力等)が_mirrorXの設定値を意識せずこのControllerと同じ変換規約に揃えられるようにする。</summary>
    public Vector2 NormalizedToViewport(float normalizedX, float normalizedY)
    {
      var x = _mirrorX ? 1f - normalizedX : normalizedX;
      var y = 1f - normalizedY;
