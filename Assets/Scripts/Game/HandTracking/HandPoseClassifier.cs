...
namespace Game.HandTracking
{
    ...
    // 5指それぞれの伸展/屈曲を判定する純粋関数群。距離の「比」だけで判定するため、
    // ワールドランドマークがどの座標系でも結果は変わらない(スケール不変)。カメラとの距離にも依存しない。
    //
    // MediaPipe GestureRecognizerの学習済みモデル(gesture_recognizer.bytes)はネイティブライブラリとの
    // バージョン不整合でロードに失敗するため(BadStatusException: hand_landmarker.task not found)、
    // HandLandmarkerの生ランドマークからこちらの自前判定でジェスチャーを組み立てる方式に戻している。
    //
    ...
    // 人差し指付け根からの距離で判定する(親指を畳む=拳を握る動作では人差し指側に寄り、伸ばす=横に開くと離れる)。
    //
    ...
    public static class HandPoseClassifier
    {
        ...
        public struct Thresholds
        {
            ...
            [Tooltip("親指を「伸展」とみなす、指先/付け根の人差し指付け根からの距離比")]
        ...
        public static FingerState GetFingerState(IReadOnlyList<Vector3> lm, Thresholds thresholds)
        {
            ...
            {
                Index = IsFingerExtended(lm[5], lm[8], palmCenter, thresholds.FingerExtendRatio),
                Middle = IsFingerExtended(lm[9], lm[12], palmCenter, thresholds.FingerExtendRatio),
                Ring = IsFingerExtended(lm[13], lm[16], palmCenter, thresholds.FingerExtendRatio),
                Pinky = IsFingerExtended(lm[17], lm[20], palmCenter, thresholds.FingerExtendRatio),
                Thumb = IsThumbExtended(lm[1], lm[4], lm[5], thresholds.ThumbExtendRatio),
            };
        }
